...
async function createMainWindow() {
  state.mainWindow = new BrowserWindow({
    ...
    frame: false, titleBarStyle: 'hidden', backgroundColor: '#0d0d0d',
    webPreferences: {
...
function createTab(url = 'https://www.google.com') {
  ...
  view.webContents.on('did-navigate',        (_, u) => { tab.url = u;   notifyShell('tab-updated', { id, url: u, title: tab.title }); });
  view.webContents.on('page-title-updated',  (_, t) => { tab.title = t; notifyShell('tab-updated', { id, url: tab.url, title: t }); });
  view.webContents.on('page-favicon-updated',(_, f) => { tab.favicon = f[0]||null; notifyShell('tab-updated', { id, favicon: tab.favicon }); });
  view.webContents.on('did-start-loading',   ()     => notifyShell('tab-loading', { id, loading: true  }));
  view.webContents.on('did-stop-loading',    ()     => notifyShell('tab-loading', { id, loading: false }));
...
function setActiveTab(id) {
  ...
  if (!tab) return;
  state.tabs.forEach(t => { if (t.id !== id) state.mainWindow.removeBrowserView(t.view); });
...
function closeTab(id) {
  ...
  else if (state.activeTabId === id) { setActiveTab(state.tabs[Math.min(idx, state.tabs.length - 1)].id); }
}
...
async function initLlama() {
  ...
  if (!fs.existsSync(modelDir)) { fs.mkdirSync(modelDir, { recursive: true }); return; }
...
async function hybridRoute(prompt) {
  ...
  } catch (err) { return { source: 'error', text: err.message }; }
}

// ── IPC Handlers ──────────────────────────────────────────────────────────
ipcMain.handle('navigate',       (_, { url }) => { const tab = state.tabs.find(t => t.id === state.activeTabId); if (!tab) return; tab.view.webContents.loadURL(url.startsWith('http') ? url : `https://${url}`); });
ipcMain.handle('tab-new',        (_, { url }) => createTab(url || 'https://www.google.com'));
ipcMain.handle('tab-close',      (_, { id  }) => closeTab(id || state.activeTabId));
ipcMain.handle('tab-switch',     (_, { id  }) => setActiveTab(id));
ipcMain.handle('tab-back',       ()           => { const t = state.tabs.find(t => t.id === state.activeTabId); if (t && t.view.webContents.canGoBack())    t.view.webContents.goBack();    });
ipcMain.handle('tab-forward',    ()           => { const t = state.tabs.find(t => t.id === state.activeTabId); if (t && t.view.webContents.canGoForward()) t.view.webContents.goForward(); });
ipcMain.handle('tab-reload',     ()           => { const t = state.tabs.find(t => t.id === state.activeTabId); if (t) t.view.webContents.reload(); });
ipcMain.handle('sidecar-toggle', (_, { open }) => { state.mainWindow.sidecarOpen = open; repositionActiveTab(); });
...
ipcMain.handle('agent-action', async (_, command) => {
  ...
  try {
    ...
    switch (command.action) {
      case 'click':    result = await tab.view.webContents.executeJavaScript(`(function(){ const el=document.querySelector(${JSON.stringify(command.selector)}); if(el){el.click();return true;} return false; })()`); break;
      case 'fill':     result = await tab.view.webContents.executeJavaScript(`(function(){ const el=document.querySelector(${JSON.stringify(command.selector)}); if(el){el.focus();el.value=${JSON.stringify(command.value)};el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));return true;} return false; })()`); break;
      case 'nav':      tab.view.webContents.loadURL(command.value); result = true; break;
      case 'scroll':   result = await tab.view.webContents.executeJavaScript(`window.scrollBy(${command.x||0},${command.y||500});true;`); break;
      case 'snapshot': result = await tab.view.webContents.executeJavaScript(`document.title+'|||'+document.body.innerText.slice(0,2000)`); break;
